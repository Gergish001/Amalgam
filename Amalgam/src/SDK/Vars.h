...
namespace Vars
{
	...
	NAMESPACE_BEGIN(Aimbot)
		SUBNAMESPACE_BEGIN(General)
			...
			CVarEnum(AimHoldsFire, 2, False, MinigunOnly, Always)
		...
		SUBNAMESPACE_BEGIN(Hitscan)
			...
			CVar(BoneSizeSubtract, 1.f)
			CVar(BoneSizeMinimumScale, 1.f)
		...
		SUBNAMESPACE_BEGIN(Projectile)
			...
			CVar(GroundStraightFuzzyValue, 100.f)
			CVar(GroundLowMinimumSamples, 16)
			CVar(GroundHighMinimumSamples, 33)
			...
			CVar(AirSamples, 33)
			CVar(AirStraightFuzzyValue, 10.f)
			CVar(AirLowMinimumSamples, 8)
			CVar(AirHighMinimumSamples, 8)
			...
			CVar(HuntsmanPullPoint, false)
			CVar(SplashPoints, 100)
			CVar(SplashGrates, true)
			CVar(SplashCountDirect, 100)
			CVar(SplashCountArc, 5)
			CVar(SplashTraceInterval, 10)
			CVarEnum(SplashMode, 0, Multi, Single)
			CVarEnum(RocketSplashMode, 0, Regular, SpecialLight, SpecialHeavy)
			CVar(DeltaCount, 5)
		...
		SUBNAMESPACE_BEGIN(Melee)
			...
			CVar(SwingTicks, 13)
			CVar(SwingPredictLag, true)
			CVar(BackstabAccountPing, true)
			CVar(BackstabDoubleTest, true)
	...
	NAMESPACE_BEGIN(Visuals)
		...
		SUBNAMESPACE_BEGIN(Trajectory)
			...
			CVar(NoSpin, false)
			CVar(LifeTime, 2.2f)
			CVar(UpVelocity, 200.f)
			CVar(AngVelocityX, 600.f)
			CVar(AngVelocityY, -1200.f)
			CVar(AngVelocityZ, 0.f)
			CVar(Drag, 1.f)
			CVar(DragBasisX, 0.003902f)
			CVar(DragBasisY, 0.009962f)
			CVar(DragBasisZ, 0.009962f)
			CVar(AngDragBasisX, 0.003618f)
			CVar(AngDragBasisY, 0.001514f)
			CVar(AngDragBasisZ, 0.001514f)
			CVar(MaxVelocity, 2000.f)
			CVar(MaxAngularVelocity, 3600.f)
		SUBNAMESPACE_END(ProjectileTrajectory)
